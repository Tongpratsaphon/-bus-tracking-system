...
// รับค่าจาก POST แทน GET เพราะเราส่งมาจาก form
$start = $_POST['start'] ?? '';
$end = $_POST['end'] ?? '';
$period = $_POST['period'] ?? '';

if (!$start || !$end || !$period) {
    echo "❌ ข้อมูลไม่ครบถ้วน กรุณากลับไปเลือกใหม่อีกครั้ง";
    exit();
}

$periodTimes = [
    'morning' => ['07:30', '07:50', '08:20', '08:40'],
    'noon' => ['12:10', '12:30', '12:50'],
    'evening' => ['16:00', '16:30', '16:50']
];
...
<body>
 ...
 <div class="side-space left"></div>
    ...
    <div class="container">
        <h2 class="text-center my-4 fw-bold">เลือกเวลาเดินทาง</h2>
        <p class="text-center fs-5 text-primary mb-4">
            ต้นทาง: <?= htmlspecialchars($start); ?> → ปลายทาง: <?= htmlspecialchars($end); ?> | ช่วงเวลา: <?= htmlspecialchars($period); ?>
        </p>
                    ...
                    <div class="col-md-6">
                        <div class="card">
                            ...
                            <form action="reservation.php" method="POST">
                                ...
                                <button type="submit" class="btn btn-time">เลือกเวลา</button>
                            </form>
                        </div>
                    </div>
                <?php endforeach; ?>
        ...
        <?php else: ?>
            <div class="alert alert-warning text-center">ไม่พบช่วงเวลาที่ระบุ</div>
        <?php endif; ?>
    </div>
</div>
